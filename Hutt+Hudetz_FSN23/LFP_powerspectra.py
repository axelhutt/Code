...
def readin(file_list):
    file_list_final = []
    for n in range(num_files):
        label = path+file_list[n]+'_'+ana+'_'+stype+'.h5'
        if file_list_flag[n] == 1:
            file_list_final.append(file_list[n])
            print("accepted:",label)
        else:
            print("rejected:",label)
    freqs,power,power_median,power_sem=D.average_powerspectra(path,ana,stype,file_list_final) # 0913B, 0824A
    return freqs,power_median

stype = 'pre'
freqs,power = readin(file_list)
power_all_list.append([freqs,power[:,:]])
